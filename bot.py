...
# Все пользователи которые писали /start
all_users = set()


async def track_message(chat_id, message_id):
    """Добавляет сообщение в трекер"""
    if chat_id not in tracked_messages:
        tracked_messages[chat_id] = []
    tracked_messages[chat_id].append((message_id, time.time()))
...
async def auto_cleanup_task():
    """Фоновая задача: каждые 4 минуты удаляет все сообщения кроме последних 2"""
    while True:
        await asyncio.sleep(240)  # 4 минуты
        for chat_id in list(tracked_messages.keys()):
            msgs = tracked_messages.get(chat_id, [])
            if len(msgs) > 2:
                to_delete = msgs[:-2]
                to_keep = msgs[-2:]
                ...
                tracked_messages[chat_id] = to_keep


async def force_cleanup(chat_id):
    """Принудительное удаление ВСЕХ сообщений в чате"""
    msgs = tracked_messages.get(chat_id, [])
...
async def track_and_send(chat_id, text, **kwargs):
    """Отправляет сообщение и трекает его"""
...
async def notify_room(room, text, keyboard_func=None, exclude=None):
    for p in room["players"]:
        ...
        try:
            ...
            msg = await bot.send_message(p, text, parse_mode="HTML", reply_markup=kb)
            await track_message(p, msg.message_id)
...
async def cmd_clearchat(message: types.Message, state: FSMContext):
    await track_message(message.chat.id, message.message_id)

    if message.from_user.id != ADMIN_ID:
        msg = await message.answer("❌ У вас нет доступа к этой команде.")
        await track_message(message.chat.id, msg.message_id)
        return

    msg = await message.answer(
        "🗑 <b>Админ запустил удаление чата.</b>\n"
        "Все сообщения из чата в течении минуты удалятся.",
        parse_mode="HTML"
    )
    await track_message(message.chat.id, msg.message_id)
    ...
    # Удаляем все трекнутые сообщения для этого чата
    await force_cleanup(message.chat.id)

    # Отправляем финальное сообщение
    final = await message.answer("✅ Чат очищен.", reply_markup=main_keyboard())
    await track_message(message.chat.id, final.message_id)
...
async def process_broadcast_message(message: types.Message, state: FSMContext):
    ...
    if not broadcast_text:
        msg = await message.answer("❌ Сообщение не может быть пустым. Попробуйте ещё раз:")
        await track_message(message.chat.id, msg.message_id)
        return
    ...
    msg = await message.answer(
        ...
        f"{broadcast_text}\n\n"
        f"👥 Получателей: <b>{len(all_users)}</b>",
        parse_mode="HTML",
...
async def cb_broadcast_send(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📨 <b>Рассылка началась...</b>", parse_mode="HTML")
...
async def cb_broadcast_cancel(callback: CallbackQuery, state: FSMContext):
    ...
    msg = await callback.message.answer("🕵️ Главное меню.", reply_markup=main_keyboard())
    await track_message(callback.message.chat.id, msg.message_id)
    await callback.answer()
...
async def menu_online(message: types.Message, state: FSMContext):
    ...
    if not nick:
        msg = await message.answer("⚠️ Сначала установите ник в настройках!", reply_markup=main_keyboard())
        await track_message(message.chat.id, msg.message_id)
        return

    msg = await message.answer("🌐 <b>Онлайн режим</b>\nВыберите действие:",
...
async def menu_local(message: types.Message, state: FSMContext):
    ...
    msg = await message.answer("📱 <b>Игра с одного телефона</b>\nВыберите количество игроков:",
...
async def menu_support(message: types.Message, state: FSMContext):
    ...
    msg = await message.answer("📞 <b>Поддержка / Предложка</b>\nНажмите кнопку ниже:",
...
async def process_nick(message: types.Message, state: FSMContext):
    ...
    if not nick:
        msg = await message.answer("❌ Ник не может быть пустым. Попробуйте ещё раз:")
        await track_message(message.chat.id, msg.message_id)
        return
...
async def process_manual_topic(message: types.Message, state: FSMContext):
    ...
    if not topic:
        msg = await message.answer("❌ Тема не может быть пустой!")
        await track_message(message.chat.id, msg.message_id)
        return
...
async def process_room_code(message: types.Message, state: FSMContext):
    ...
    if not text.isdigit() or len(text) != 3:
        msg = await message.answer("❌ Введите 3-значный числовой код:")
        await track_message(message.chat.id, msg.message_id)
        return
    ...
    if not found_room:
        msg = await message.answer("❌ Комната не найдена или уже заполнена. Попробуйте ещё раз:")
        await track_message(message.chat.id, msg.message_id)
        return
    ...
    msg = await message.answer("✅ Вы вступили в комнату!")
    await track_message(message.chat.id, msg.message_id)
...
async def cb_start_game(callback: CallbackQuery, state: FSMContext):
    ...
    room["spies"] = random.sample(room["players"], 1)
    ...
    for p in room["players"]:
        try:
            if p == current_player:
                ...
                msg = await bot.send_message(p, text, parse_mode="HTML")
            await track_message(p, msg.message_id)
        except:
            pass
...
async def handle_in_room_message(message: types.Message, state: FSMContext):
    ...
    if uid != current_player:
        msg = await message.answer("⚠️ Сейчас не ваш ход! Дождитесь своей очереди.")
        await track_message(message.chat.id, msg.message_id)
        return
    ...
    for p in room["players"]:
        try:
            ...
            await track_message(p, m.message_id)
        except:
            pass
...
async def finish_game(room):
    ...
    for p in room["players"]:
        try:
            ...
            msg = await bot.send_message(p, result_text, parse_mode="HTML", reply_markup=kb)
            await track_message(p, msg.message_id)
        except:
            pass
    ...
    if uid in user_room:
        ...
        if rid in rooms:
            ...
            if room["started"] and not room.get("lobby_over"):
                if room.get("voting_phase"):
                    msg = await message.answer("⚠️ Сейчас голосование! Используйте кнопки.")
                    await track_message(message.chat.id, msg.message_id)
                    return
                current_player = room["turn_order"][room["current_turn"]] if room["current_turn"] < len(
                    room["turn_order"]) else None
                if uid != current_player:
                    msg = await message.answer("⚠️ Сейчас не ваш ход!")
                    await track_message(message.chat.id, msg.message_id)
                    return

    msg = await message.answer("Используйте кнопки меню или /start", reply_markup=main_keyboard())
    await track_message(message.chat.id, msg.message_id)
...
async def main():
    # Запускаем фоновую задачу авто-очистки
